...
for i=1:length(S_calc_sortBIC)
    ...
    for m=1:length(M.experiment)
        % determin order of experimenral conditions for plot
        order=[ 4 3 2 1 5 ];
        
        
        if length(S_calc_sortBIC{1,i}.model.experiment(1,order(m)).component_index)==1
            if S_calc_sortBIC{1,i}.model.experiment(1,order(m)).component_index(1)==2  % no Wt
                
                aa(1)=plot(i,m+0.05,'ko','Markersize',7); hold on;
                
            end
            
            if S_calc_sortBIC{1,i}.model.experiment(1,order(m)).component_index(1)==1  % no SAC_pop
...
ylim([min(BIC_sort)-20 5.3293e+03]);
